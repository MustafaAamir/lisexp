...
Copyright © 2019 Jesper Oskarsson
*)
...
let slurp_until_end_quote_and_give_me_the_string rest_of_quoted_symbol =
  let rec inner : string -> string -> string =
    ...
    | '\'' -> "\"" ^ symbol_buffer ^ "\""
    | _ -> inner (rest src_str) (str [ symbol_buffer; String.make 1 src_str.[0] ])
  in
    ...
    then EndOfTransmission :: acc
      ...
      | '\'' ->
        ...
        let src_with_symbol_cut_off =
          String.sub src symbol_length (String.length src - symbol_length (* - 2 *))
        in
...
let parse_list iterator parse_s_expressions =
  ...
  | next_iterator, expressions -> ParseNext (next_iterator, SexpList expressions)
;;
...
let s_expression_of_token_list : token list -> s_expression =
  ...
  let rec parse_s_expressions
    ...
    | ParseNext (iterator, result) -> parse_s_expressions iterator (result :: expressions)
  in
