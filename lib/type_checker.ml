   ...
   | TUserDefined of string * ty list (* User-defined type with parameters *)
               ...
               | TInt | TFloat | TBool | TString -> IntSet.empty
                            | TList t -> free_vars t
                             | TFun (args, ret) ->
let init' = free_vars ret in let f' acc arg = IntSet.union acc (free_vars arg) in
...
let subst = List.zip_exn (IntSet.to_list free) (List.map vars ~f:(fun v -> TPoly (v, TVar 0))) in
