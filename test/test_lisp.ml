...
let value_tests = "value_tests" >::: [
  ...
  "test_list" >:: (fun _ ->
    let env = setup_test_env () in
    test_eval env "(list 1 2 3)" (List [Integer 1; Integer 2; Integer 3]));
]

let arithmetic_tests = "arithmetic_tests" >::: [
  "test_addition" >:: (fun _ ->
    let env = setup_test_env () in
...
let comparison_tests = "comparison_tests" >::: [
  "test_equal" >:: (fun _ ->
    let env = setup_test_env () in
...
let list_operation_tests = "list_operation_tests" >::: [
  "test_car" >:: (fun _ ->
    let env = setup_test_env () in
  ...
  "test_cons" >:: (fun _ ->
    let env = setup_test_env () in
    test_eval env "(cons 1 (list 2 3))" (List [Integer 1; Integer 2; Integer 3]));
...
let function_tests = "function_tests" >::: [
...
let higher_order_function_tests = "higher_order_function_tests" >::: [
  "test_map" >:: (fun _ ->
    let env = setup_test_env () in
  ...
  "test_filter" >:: (fun _ ->
    let env = setup_test_env () in
    let _ = eval_string env "(define even? ($ (x) (= (/ x 2) (* (/ x 2.0)))))" in
...
let error_handling_tests = "error_handling_tests" >::: [
  "test_undefined_symbol" >:: (fun _ ->
    let env = setup_test_env () in
    assert_raises (Failure "Undefined symbol: xyz")
      (fun () -> eval_string env "xyz"));

  "test_type_error" >:: (fun _ ->
    let env = setup_test_env () in
    assert_raises (Failure "Invalid arguments to +")
      (fun () -> eval_string env "(+ 1 'hello')"));

  "test_arity_error" >:: (fun _ ->
    let env = setup_test_env () in
    let _ = eval_string env "(define f ($ (x) x))" in
    assert_raises (Failure "Argument count mismatch dumbass")
      (fun () -> eval_string env "(f 1 2)"));
]
